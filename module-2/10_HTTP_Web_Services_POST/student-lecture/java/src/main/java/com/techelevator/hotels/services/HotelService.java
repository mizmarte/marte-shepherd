...
public class HotelService {
	...
	public Reservation addReservation(String newReservation)
	{
		...
		//new reservation - 1, Marte' Shepherd, 2/1/2021, 2/5/2021,2
		...
		headers.setContentType(MediaType.APPLICATION_JSON); // tell server to expect JSON
	...
	public Reservation updateReservation(String CSV) 
	{
		...
		//passing in all information for the update
		//build URL
		String url = BASE_URL + "reservations/" + reservationToUpdate.getId();
		
		//append the id as the wehere clause for the update statement
		//create headers
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON);//what data type to expect in teh body of rquest
		
		HttpEntity<Reservation> entity = new HttpEntity<Reservation>(reservationToUpdate, headers);
		
		try
		{
			//put does not return a response for object
